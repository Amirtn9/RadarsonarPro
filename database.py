...
class Database:
    def __init__(self):
        ...
        # ✅ اصلاح: ایجاد Connection Pool برای مدیریت بهینه اتصالات
        # minconn=1: حداقل اتصالات باز
        # maxconn=20: حداکثر اتصالات همزمان (بسته به قدرت سرور قابل تغییر است)
        try:
            self.pool = psycopg2.pool.ThreadedConnectionPool(1, 20, **self.db_config)
    ...
    def get_connection(self):
        """مدیریت اتصال به PostgreSQL با استفاده از Connection Pool"""
        conn = None
        try:
            # ✅ اصلاح: دریافت اتصال از استخر به جای ساخت اتصال جدید
            ...
            if conn:
                # ✅ اصلاح: بازگرداندن اتصال به استخر (نه بستن آن)
                self.pool.putconn(conn)

    def execute(self, query, params=None):
        """
        متد کمکی برای اجرای کوئری‌های ساده (UPDATE/DELETE/INSERT بدون نیاز به نتیجه)
        ⚠️ توجه: چون کانکشن در پایان with به pool برگردانده می‌شود، دیگر cursor
        را برنمی‌گردانیم؛ چون استفاده از آن بعد از این متد باعث خطا/رفتار نامعتبر می‌شود.
        اگر جایی نیاز به نتیجه (fetch) دارید، مستقیماً از db.get_connection() استفاده کنید.
        """
